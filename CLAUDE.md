...
测试内联在各自模块的 `#[cfg(test)] mod tests` 中，另有 `tests/integration_test.rs`（~1350 行，20 个端到端测试，使用 `tests/fixtures/` 下的日志文件和 `expected.json` 做数据驱动断言）。
